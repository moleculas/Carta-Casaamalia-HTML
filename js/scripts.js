...
const gestionaCarta = (res, idioma, objeto) => {
    ...
    const propietatsPerIdioma = {
        ...
        ...categoriesCarta.plats.reduce((obj, categoria) => {
            obj[`nomsItemsCategoria${categoria}`] = res.items
                .sort((a, b) => a.ordre - b.ordre)
                .filter((item) => item.categoria === categoria)
                .map((item) => item[`nom_${idioma}`]);
            return obj;
        }, {}),
    ...
    categoriesCarta[objeto].forEach((_, index) => $(`#menu-section-${objeto === "plats" ? index : index + 1}`).append(categoriaAfegir[index]));
...
function initRestabook(divsToggleP, divsToggleA, idioma, PARADES, PRODUCCIO) {
    ...
    $("#alergens").click(function () {
        ...
        if ($(this).hasClass('alergens-activat')) {
            ...
            $('.descripcio').removeClass('marcadors1 marcadors2 marcadors3 marcadors4');
